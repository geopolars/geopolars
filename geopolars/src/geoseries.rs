...
use polars::export::arrow::array::{
    Array, BinaryArray, BooleanArray, ListArray, MutableBinaryArray, MutableBooleanArray,
    MutablePrimitiveArray, PrimitiveArray, StructArray,
};
use polars::prelude::{NamedFromOwned, PolarsError, Series};
...
impl GeoSeries for Series {
    ...
    //     // let chunk = &self.0.chunks()[0];
    //     // let struct_arrow_array = chunk.as_any().downcast_ref::<StructArray>().unwrap();
    //     // let struct_array_values = struct_arrow_array.values();
    //     // let x_arrow_array = &struct_array_values[0];
    //     // let y_arrow_array = &struct_array_values[1];

    //     // let x_array_values = x_arrow_array.as_any().downcast_ref::<PrimitiveArray<f64>>().unwrap();
    //     // let y_array_values = y_arrow_array.as_any().downcast_ref::<PrimitiveArray<f64>>().unwrap();

    //     // let tmp = x_array_values.value(i);
    //     // let tmp = x_array_values[0];
