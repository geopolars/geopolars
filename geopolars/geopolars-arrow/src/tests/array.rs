...
use crate::to_arrow::polars_array_to_arrow;
use crate::to_geoarrow::polars_array_to_geoarrow;
use crate::to_polars::{arrow_array_to_polars, geoarrow_array_to_polars};
...
fn round_trip_integer() {
    ...
    let polars_array = arrow_array_to_polars(&array, &field);
    let (back_array, back_field) = polars_array_to_arrow(polars_array);
...
fn round_trip_point() {
    for coord_type in [CoordType::Interleaved, CoordType::Separated] {
        for dim in [
            ...
            let polars_array = geoarrow_array_to_polars(&array);
            let back = polars_array_to_geoarrow(polars_array);
...
fn round_trip_wkb() {
    ...
    let polars_array = geoarrow_array_to_polars(&wkb_array);
    let back = polars_array_to_geoarrow(polars_array);
